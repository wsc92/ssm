    ...
    def run(self):
        ...
        if output:
            ...
            if banned_match and total_banned_match:
                ...
                info_msg = f"fail2ban is active: {currently_banned} currently banned, {total_banned} total banned"

                # Get list of banned IPs if any exist
                if currently_banned > 0:
                    banned_ips = self._get_banned_ips("sshd")
                    if banned_ips:
                        info_msg += f" - Banned IPs: {', '.join(banned_ips)}"

                self.result.info.append(info_msg)
