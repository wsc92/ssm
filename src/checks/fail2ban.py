...
class Fail2BanCheck(BaseCheck):
    ...
    def run(self):
        ...
        if output:
                ...
                if currently_banned > 0:
                    ...
                    if banned_ips:
                        self.result.info.append(
                            f"fail2ban: {currently_banned} IPs currently banned (of {total_banned} total):"
                        )
                        for ip in banned_ips:
                            self.result.info.append(f"  • {ip}")
                    else:
                        self.result.info.append(
            ...
            self.result.info.append("fail2ban is active (unable to query jail status)")
